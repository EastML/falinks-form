 ...
 */
export const isRequired = (text) => text.trim() === "" && "Input required";
/**
 * Checks if a string exceeds some arbitrary amount of characters
 * @param {number} limit
 * @returns A failed validation message or false.
 */
export const maxLength = (limit) => (text) => text.length > limit && `Input too long. (${text.length} / ${limit}) characters`;
/**
 * Checks if a string is below some arbitrary amount of characters
 * @param {number} limit
 * @returns A failed validation message or false.
 */
export const minLength = (limit) => (text) => text.length < limit &&
 ...
 * @returns A failed validation message or false.
 */
export const isNumber = (text) => isNaN(Number(text)) && 'Input must be a number';
 ...
 * @returns A failed validation message or false
 */
export const isEmail = (text) => !email(text) && 'Invalid email format (ex: person@email.com)';
/**
 ...
 * @returns A failed validation message or false
 */
export const matches = (checkedValue) => (value, form) => value !== form[checkedValue] && "Values do not match";
